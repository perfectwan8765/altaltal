...
%>
	...
	%>
			<tr align='center'>
				<td><%=number%></td>
				<td><%=commentList.get(i).get("place_area").toString() %></td>
				<td><%=commentList.get(i).get("place_name").toString() %></td>
				<td><%=commentList.get(i).get("pboard_taste").toString() %>점</td>
				<td><%=commentList.get(i).get("pboard_price").toString() %>점</td>
				<td><%=commentList.get(i).get("pboard_service").toString() %>점</td>
				<td><%=sdf.format((Date)commentList.get(i).get("pboard_date")) %></td>
	...
	<div class="container">
          <form class="form-inline" action="./myPlaceCommentList.my" method="POST" onsubmit="return searchCheck();" >
              <center>
                          <select class="form-control" name="topic" id="topic">
                              <option value="name" >맛집명</option>
                              <option value="area" >맛집 지역</option>
                          </select>
	...
	<ul class="pagination">
		...
		<li><a>«</a></li>&nbsp;
		<%}else{ %>
		<li><a href="myPlaceCommentList.my?page=<%=nowPage-1 %>&keyword=<%=keyword%>&topic=<%=topic%>">«</a></li>&nbsp;
		<%} %>
		...
		<li><a><%=a %></a></li>
		<%}else{ %>
		<li><a href="myPlaceCommentList.my?page=<%=a %>&keyword=<%=keyword%>&topic=<%=topic%>"><%=a %>
		</a></li>&nbsp;
		<%} %>
		<%} %>
